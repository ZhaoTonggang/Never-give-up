...
const zoom = () => {
	const WinHeight = 670,
		WinWidth = 1176;
	scale = Math.min(window.innerHeight / WinHeight, window.innerWidth / WinWidth);
...
//最高关卡
var highest = location.hash.slice(1).split('/')[3] || 1;
//挑战模式的最高记录
var max = 0;
//背景音乐
// 图片
...
function indexFn() {
	...
	var mode = location.hash ? location.hash.slice(1).split('/')[0] : 0;
	var charact = location.hash ? location.hash.slice(1).split('/')[1] : 'whiteC';
	var levels = location.hash ? location.hash.slice(1).split('/')[2] : '1';
	//选择人物
		...
		moving({
			...
			callBack: function() {
				...
				stars.timer = setInterval(function() {
					...
					ii.style.background = 'url(' + urls[Math.round(Math.random())] +
						') no-repeat center';
...
function chooseCharacter(obj) {
	var hash = location.hash ? location.hash.slice(1).split('/')[1] : 'whiteC';
	switch (hash) {
		case 'whiteC':
...
function levelFn() {
	...
	var hash = location.hash ? location.hash.slice(1).split('/') : null;
...
function mainFn() {
	...
	//获取hash
	var nums = location.hash.slice(1).split('/')[2];
	var charc = location.hash.slice(1).split('/')[1];
	var hash0 = location.hash.slice(1).split('/')[0];
	...
	function checkpoint(j) {
		...
		function rander(num) {
			while (objNum < num * fre) {
				...
				switch (Data[objNum]) {
					...
					case 3:
						li.innerHTML =
							'<div> <span class="y"></span><i class="green di_gb"></i></div><strong class="di"></strong>'
						li.className = '_3';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 4:
						li.innerHTML =
							'<div><span class="fire"></span><i class="red di_gb"></i></div><strong class="di"></strong>'
						li.className = '_4';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 5:
						li.innerHTML =
							'<div><span class="y"></span><i class="green zhong_bg"></i></div> <strong class="zhong"></strong>'
						li.className = '_5';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 6:
						li.innerHTML =
							'<div><span class="fire"></span><i class="red zhong_bg"></i></div><strong class="zhong"></strong>'
						li.className = '_6';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 7:
						li.innerHTML =
							'<div><span class="y"></span><i class="green gao_bg"></i></div> <strong class="gao"></strong>'
						li.className = '_7';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 8:
						li.innerHTML =
							'<div><span class="fire"></span><i class="red gao_bg"></i></div><strong class="gao"></strong>'
						li.className = '_8';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 9:
						li.innerHTML =
							'<div><span class="y"></span><i class="green"></i></div><div><em></em><i class="red huo_d"></i></div>'
						li.className = '_9';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 10:
						li.innerHTML =
							'<div><span class="y"></span><i></i></div><div><em></em><i class="red huo_g"></i></div>'
						li.className = '_10';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 11:
						li.innerHTML =
							'<div><span class="y"></span><i class="green zhong_bg"></i></div><strong class="zhong"></strong><div><em></em><i class="red huo_g"></i></div>'
						li.className = '_11';
						li.style.left = objNum * 100 + 300 + 'px';
						break;
					case 0:
		...
		function jump(obj, novice) {
			...
			if (novice) {
				//检测后4个是否为需要跳的
				if ((lis[n + 4] && lis[n + 4].height > lis[n].height && lis[n + 4].className != '_0') || (lis[n + 3] &&
						lis[n + 3].height > lis[n].height && lis[n + 3].className != '_0') || (lis[n + 2] && lis[n + 2]
						.height > lis[n].height && lis[n + 2].className != '_0')) {
				...
				//检测是否是两个连柱
				if (((lis[n + 2] && lis[n + 2].height) > lis[n].height) && ((lis[n + 2] && lis[n + 2].height) < (lis[n +
						3] && lis[n + 3].height))) {
					p.innerHTML = '双击空格';
				}
				//播放倍数
		...
		function handleStartInput() {
			if (boss.style.display !== 'block') return;
			...
			if (star) {
				...
				// 墨水机消失动画
				animateElement(trigger, -3.1);

				// 关卡信息消失动画
				animateElement(explain, -15);

				jump(boss, j.novice);
				blackBall();

				// 播放背景音乐
				const bgmToPlay = play === 1 ? bgm : nbgm;
		...
		// 通用元素动画函数
		function animateElement(element, speed) {
			const timer = setInterval(() => {
				const l = element.offsetLeft;
				element.style.left = l + speed + 'px';

				if (l + element.offsetWidth <= 0) {
					clearInterval(timer);
				}
			}, 10);
		}
				...
				timer = setInterval(function() {
					...
					if (t >= tagerNext) {
						...
						if (m >= Data.length - 1) {
							...
							location.hash = hash0 + '/' + charc + '/' + nums + '/' + highest + '/' + percentage
								.innerHTML;
